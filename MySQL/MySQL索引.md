...
4、数据量大时，可能无法一次性加载到内存中

**不用B树的原因**：

1、虽然B树可通过中序遍历进行范围查询，但需要不断回到父结点，访问路径更复杂

2、B树的非叶子节点存储了数据，所以一次可加载到内存中的节点更少，增加了IO次数
...
2、B+树的非叶子节点不存储数据，所以可以一次加载更多节点到内存中，查询速度比较稳定（高度一般在3-4层）
...
- 聚簇索引只有一个；非聚簇索引可以有多个
- 聚簇索引叶子节点存储的是整行数据；非聚簇索引叶子节点存储的是索引+主键
- 聚簇索引查询速度快，只需要查询一次即可找到数据；非聚簇索引查询速度慢，需要先查询一次找到主键，再由聚簇索引查询一次找到数据
- 聚簇索引的更新代价大（因为更新索引键会导致数据行在 B+ 树中移动）；非聚簇索引代价小（因为更新只需修改索引项）
...
- **避免索引失效**：复合索引调整索引列的顺序（或者修改查询语句）
- **避免回表**：增加必要的字段到索引中
- **单机环境尽量使用自增主键**：防止页分裂等，详细见另一篇文章
...
- 将 WHERE 条件的过滤操作从服务器层下推到存储引擎层
- 减少不必要的数据传输和回表操作
- 特别适用于复合索引中只有部分列用于查找的场景
- 在 MySQL 5.6+ 中默认启用，通常能显著提升查询性能
