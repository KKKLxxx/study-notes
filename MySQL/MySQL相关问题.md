...
如果是**分布式环境**，往往要考虑ID冲突。UUID是指Universally Unique Identifier，翻译为中文是通用唯一识别码，UUID的目的是让分布式系统中的所有元素都能有唯一的识别信息。如此一来，每个人都可以创建不与其它人冲突的UUID，就不需考虑数据库创建时的名称重复问题。所以分布式环境下往往会放弃一些性能，采用UUID

由于UUID是随机的，所以用UUID作为主键进行插入很容易产生页分裂。在分布式环境中，一般采用**雪花ID**来替代UUID。雪花ID的生成与时间相关，所以天然保证了有序性，从而避免页分裂，但性能相比自增主键来说还是要差一点
