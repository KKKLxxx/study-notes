...
- **在数据修改后，将修改记录写入Buffer，在事务提交后，根据配置决定持久化时机**
- 由参数 `sync_binlog` 控制：
  - `0`：交给操作系统控制（高性能，不安全）。
...
- **容量满删除**：根据配置的总大小（`max_binlog_size`）自动清理。（主从复制是快照全量同步+Binlog增量同步实现的）
...
- **物理日志 (Physical Log)**：特指 **Redo Log**。它记录的是在“某个数据页（Page）的某个偏移（offset）”上的修改（例如：在第 5 号页面的偏移量 100 处写入了什么字节）。
- **逻辑日志 (Logical Log)**：Binlog（包括 Row 格式）和Undo Log。因为它记录的是SQL语句或数据行的内容变更，不涉及物理存储位置（Page/Offset）。

### 2. Binlog 与 Redo Log 的区别（能否用 Redo Log 进行主从复制？）

**主要区别：**

| **特性**     | **Binlog**                        | **Redo Log**                  |
| ------------ | --------------------------------- | ----------------------------- |
| **产生位置** | MySQL Server 层                   | InnoDB 引擎层                 |
| **主要用途** | 主从复制、数据恢复                | 保证事务的持久性              |
| **记录内容** | **逻辑日志** (SQL 语句或行数据)   | **物理日志** (数据页的修改)   |
| **写入方式** | **追加写** (文件只会增加，不覆盖) | **循环写** (固定大小，会覆盖) |
