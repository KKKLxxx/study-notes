...
指当一条线程修改了变量的值，新值对于其他线程来说是可以立即得知的

**实现原理**：对于用volatile修饰的变量，对其写操作时会**额外执行一个Lock指令**，这个Lock指令的作用如下
...
在单线程环境下，编译器和处理器为了提高运行速度，会在不影响运行结果的前提下对指令进行重排序。而在多线程环境下，这种重排序就会造成错误的结果

**实现原理**：JMM通过**在修改volatile变量的指令之后插入一个内存屏障，来禁止重排序时内存屏障之后的指令被重排序到内存屏障之前**
...
二、volatile与synchronized的对比
---------------

- **使用方法**：volatile只能修饰变量；synchronized可以修饰方法和代码块
- **作用**：volatile不能保证线程安全，而synchronized可以

三、volatile与synchronized的选择
--------------------------

一句话总结：**如果只是希望别的线程立即看到（即只读）变量值的变化，则用volatile；否则都用synchronized等锁机制**
...
而在像如下所示的这类场景中就很适合使用volatile变量来控制并发，当shutdown()方法被调用时，能保证所有线程中执行的doWork()方法都立即停下来
