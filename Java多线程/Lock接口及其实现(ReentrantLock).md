...
`Lock`接口的功能与`synchronized`类似，都是用来控制多个线程访问共享资源的方式。**`synchronized`的优势在于隐式获取释放锁的便捷性**，不用考虑异常等特殊情况；但是**`Lock`也有`synchronized`不具备的特性，能够更灵活地使用**（具体区别见ReentrantLock与synchronized对比）
...
| **void lockInterruptibly() throws InterruptedException**     | 可中断地获取锁，即在锁的获取过程中可以中断当前线程           |
| **void unlock()**                                            | 释放锁                                                       |
