...
**CAS的思想**：向CAS方法传入预期的旧值A和即将更新的值B，当且仅当预期的旧值和实际的旧值相同时，才将内存值修改为B并返回true，否则什么都不做，并返回false
...
因为在锁竞争较小的情况下，竞争时间不会持续很长，为了这段时间去挂起和恢复线程并不值得。所以可以通过自旋的方式让后面的线程等待并得到快速的响应。但如果条件一直不满足，CAS会持续自旋，浪费CPU资源

（这里的自旋并不是指CPU的CAS操作会自旋，而是在业务层面通过校验CAS的返回值进行的循环操作）
