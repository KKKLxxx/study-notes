...
synchronized是一种锁机制，可以保证线程安全，可以修饰代码块或方法

Java中的每一个对象都可以作为锁，具体表现为以下3种形式

- 对于**同步代码块**，锁是synchronized括号里配置的对象
- 对于**普通同步方法**，锁是当前实例对象
...
- 对于代码块，synchronized关键字经过Javac编译之后，会在代码块的前后分别形成monitorenter和monitorexit这两个字节码指令。monitorenter插入到代码块的开始位置，monitorexit插入到代码块结束处和异常处。JVM保证每个monitorenter必须有对应的monitorexit与之配对，但可能不止一个monitorexit（正如此例）
...
**但从本质上来说，对于代码块和方法，都是基于对一个对象的monitor的获取与释放来实现同步（Java中的每个对象都有一个monitor与之关联，如果一个monitor被持有，那么对象就处于锁定状态，其他线程就会被阻塞），只不过两者的实现细节不同**
