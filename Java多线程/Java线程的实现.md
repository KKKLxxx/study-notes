...
阻塞状态会在锁释放后参与锁竞争，而等待状态会等到满足某个条件后再参与锁竞争，所以等待的好处是能够避免不必要的锁竞争，但阻塞能够更快地响应
...
线程池可以调用shutdown()或者shutdownNow()方法关闭线程池

## 七、协程与多线程

协程是用户级线程，一个内核级线程上可以运行多个协程，协程的创建和调度由应用程序实现，从而减少了系统调用

协程的资源占用更小，所以上下文切换的开销也会更小，更适合IO密集型任务

但协程一定要配合非阻塞IO或异步IO使用，并且要在不满足条件时主动让出CPU（yield）。否则一个协程阻塞了，会导致同一个线程上的其他协程也阻塞，就无法充分利用协程的优势

Java使用内核级线程是由于历史原因（更好实现），但JDK21发布了虚拟线程的新特性，所以未来Java也将能够使用协程
