...
可以将cookie附加在URL后面或请求体中

## 3. Token

token主要用于解决session的缺陷

解决方式：用户第一次请求登录时，即加密生成一个token并返回给用户。用户将token保存在cookie中。之后的每次请求，服务器都会先验证cookie中的token是否合法

这种方式相当于用CPU计算代替了大量的空间存储，而且通过这种方式很容易扩展应用

**但是为了实现“强制下线”、“监控在线状态”等功能，往往还是需要在服务端存储token，这里可以通过引入Redis解决**
