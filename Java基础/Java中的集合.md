...
`CopyOnWriteArrayList`是一种**线程安全**的`ArrayList`，适用于**读多写极少的并发场景**

**读操作：通过volatile修饰底层的数组保证线程间的可见性，从而实现不加锁的读取**
...
**写操作：会先加可重入锁，再将当前数组复制一份，在新数组上执行写操作，最后将原数组覆盖，并释放锁**
...
2、可能读取到旧数据：在数据已经写入新数组，但还没来得及覆盖时进行读操作，则会读取到旧数据。虽然不是最新的数据，但仍然是线程安全的，因为线程安全并不要求每个线程必须实时看到最新数据，CopyOnWriteArrayList是一种弱一致性模型

**既然加锁了，为什么不能直接在原数组上进行修改，而要再复制一份：因为读操作是不加锁的，如果直接在原数组上修改，读操作可能读取到正在修改中的数据（扩容场景），而这个数据可能是不完整的**
