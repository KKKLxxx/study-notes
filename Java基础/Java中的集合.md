...
**读操作**：直接进行读取。底层数组用volatile保证了线程可见性，不用加锁
...
**写操作**：比如向容器中添加一个元素，则会先加锁，再将当前数组复制一份，在新数组上执行写操作，最后将原数组覆盖
...
**既然加锁了，为什么不能直接在原数组上进行修改，而要再复制一份：因为读操作是不加锁的，如果直接在原数组上修改，读操作可能读取到正在修改中的数据，这个数据可能是不完整的**
