...
**实现原理**：JVM在第一次加载某个类时会生成一个Class对象，里面记录了这个类的信息

**使用场景**：

- **动态代理**：通过反射获取被代理类的字段、方法等信息，从而在运行时动态地创建代理对象

- **Spring注解**：通过反射在运行时检查字段、方法上的注解信息，并执行相应逻辑（如@Value加载配置文件，@Component创建Bean）
...
* **安全**：反射破坏了抽象性，它能够访问私有的变量或方法

## 二、动态代理

**动态代理的作用**：将与业务逻辑无关的代码抽离出来，从而达到解耦、复用的目的。如权限检验、日志收集等

动态代理的实现有两种：JDK代理和CGLIB代理

### 1. JDK代理

**实现方法**：JDK代理是基于反射实现的。它使代理类继承于一个Proxy类，从而持有InvocationHandler属性。对代理对象的方法调用都会交由InvocationHandler的invoke方法处理。invoke方法接受3个参数：代理对象、方法、参数列表。重写invoke方法便可以在原方法的基础上添加其他处理逻辑
...
**实现方式**：CGLIB代理是基于字节码实现的。它通过字节码生成代理类，从而使代理类直接继承于被代理类，所以可以代理类方法而非仅仅接口方法
...
| 代理方式       | JDK代理                 | CGLIB代理              |
| -------------- | ----------------------- | ---------------------- |
| **可代理方法** | 仅可代理接口方法        | 可代理接口方法与类方法 |
| **实现方式**   | 反射                    | 字节码                 |
| **依赖**       | `java.lang.reflect`自带 | 需要引入CGLIB库        |

**两者性能上的差别不大，在选择时，主要还是考虑被代理方法的类型，而不是速度**
