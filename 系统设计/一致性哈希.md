...
（一致性哈希算法就是通过哈希环来减少存储节点变化时的数据迁移量，并通过虚拟节点使数据分布更加均匀。它的原理就是将数据和节点根据哈希值分散到环的不同位置上，每个数据会存储到它顺时针方向的下一个节点，这样在节点数量变化时，只会影响少数几个节点，而不会像传统的取模方式可能影响几乎所有节点。而虚拟节点的作用就是增加节点的数量，从而使数据分布更均匀，多个虚拟节点会映射到一个物理节点上）
...
到这里一致性哈希的基本原理已经介绍完了，但如果哈希函数的分布不均匀，仍有可能造成某个节点压力过大的问题
...
同时，可以根据物理节点的性能，设置不同的虚拟节点的数量，使系统更灵活，稳定性更高
