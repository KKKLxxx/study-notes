...
- **支持动态扩容**：由SDS自行管理内存，并通过预分配减少内存分配次数；执行字符串拼接等操作时，会校验是否会使缓冲区溢出，并自动扩容

## 二、数字集合intset

intset是一个由整数组成的有序数组，从而便于在上面进行二分查找，用于快速地判断一个元素是否属于这个集合
...
压缩列表是用一块连续的内存，为不同大小的元素分配尽可能少的空间，以节省内存。所以每个元素需要一个头部来存储元素所占的大小，但是也因此无法进行随机读取，只能通过遍历寻找某一元素

因此压缩列表**读取操作的时间复杂度为O(n)，仅适合用于数据量较少的场景**
...
1. 初始状态：假设一个 ziplist 中，有多个长度在 250～253 字节之间的节点（记为 E1, E2, ..., EN）。由于前一节点长度 < 254，所以每个节点的 `previous_entry_length` 都只占 1 字节。
2. 插入“大节点”：在 E1 之前插入一个 260 字节的新节点（记为 New）。New 需要成为 E1 的“前一个节点”。
3. E1 的更新：E1 发现自己的 `previous_entry_length` 需要记录 New 的长度（260 ≥ 254）。因此，E1 的 `previous_entry_length` 字段必须从 1 字节扩展为 5 字节。
4. 连锁反应：
   - E1 的长度因此从 253 字节变为 257 字节。
   - 现在，E2 发现自己的“前一个节点”E1 的长度变为了 257（≥ 254），于是 E2 的 `previous_entry_length` 字段也需要从 1 字节扩展为 5 字节。
   - 这个扩展效应会沿着列表向后传播，直到某个节点在更新后，其长度依然小于 254 字节，连锁反应才会停止。

这种扩展使得每个受影响的节点都需要触发一次内存重分配和数据移动，形成“连锁更新”
...
**listpack更新元素时，虽然也可能导致后面的元素需要移动，但是这个移动是一次性的，在内存中只需要一次拷贝。而连锁更新指的是，由于第一个`previous_entry_length`的变化，导致第二个元素的`previous_entry_length`也要修改，接着又可能导致第三个`previous_entry_length`也要修改，这样修改次数会很多，不是一次性完成的**
...
（快速列表由双向链表和紧凑链表组成，双向链表的特点是每个节点都有一个向前和向后的指针，所以便于节点的插入和删除，但是缺点在于由于每个节点都要存储这两个指针，所以空间利用率比较低，而且会有内存碎片问题。而紧凑链表是用一块连续的空间去存储多个元素，从而提高了内存利用率，并且缓解了内存碎片问题）
...
从上面我们可以知道，跳表在链表的基础上增加了多级索引以提升查找的效率，但其实是一个空间换时间的方案，必然会带来一个问题——索引是占内存的。原始链表中存储的有可能是很大的对象，而索引结点只需要存储关键值和几个指针，并不需要存储对象，因此**当节点本身比较大或者元素数量比较多的时候，其优势必然会被放大，而缺点则可以忽略**
