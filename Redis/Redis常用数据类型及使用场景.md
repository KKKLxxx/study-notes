...
Redis常见的数据类型有5种：**String（字符串）、List（列表）、Hash（哈希）、Set（集合）、Zset（有序集合）**
...
分布式锁中，需要利用Hash类型来同时保存拥有者和获取次数两个属性
...
HyperLogLog 用于基数统计，就是统计不重复的元素个数，但要注意，HyperLogLog 是统计规则是基于概率完成的，不是非常准确，标准误算率是 0.81%
...
（Stream是一个轻量可靠的消息队列，它相比Redis之前基于List或发布订阅的消息队列，主要是通过消息持久化和ACK机制来保证了可靠性。但是它相比RocketMQ这种专业的消息队列仍有一些缺陷，比如Stream基于内存，消息积压时可能导致OOM，以及不支持延迟消费、事务消息等高级功能）
