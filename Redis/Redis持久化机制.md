...
Redis 是先执行写操作命令后，才将该命令记录到 AOF 日志里的。这种形式避免阻塞了当前命令，但如果在记录日志之前发生宕机可能造成数据丢失
...
具体方式为，读取日志中key的最新value，生成一条写命令保存到一个新的AOF文件中，最后将旧的文件覆盖
...
混合持久化是在 **AOF 重写时**，将当前数据库快照以 RDB 格式写入新 AOF 文件头部，再在尾部以 AOF 格式写入增量的命令，最后覆盖旧 AOF 文件
